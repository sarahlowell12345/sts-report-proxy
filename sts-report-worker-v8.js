function getHTML() {
  ...
  parts.push('<div class="stat-chip alert"><div class="val" id="o-neg-reviews"></div><div class="lbl">Negative reviews</div></div>');
  parts.push('</div>');
  ...
  parts.push('var repGmb=(d.reputation&&d.reputation.gmb)||{};var allReviews=repGmb.negativeReviews||[];');
  parts.push('var totalReviews=d.gmbTotalRatings||0;');
  parts.push('var repliedReviews=repGmb.repliedReviews!=null?repGmb.repliedReviews:0;');
  parts.push('var negReviews=repGmb.negative||allReviews.length||0;');
  parts.push('var replyRate=repGmb.reviewReplyPercentage!=null?Math.round(repGmb.reviewReplyPercentage):(totalReviews>0&&repliedReviews>0?Math.round((repliedReviews/totalReviews)*100):null);');
  ...
  parts.push('document.getElementById("o-reply-rate").textContent=replyRate!==null?replyRate+"%":"--";');
  parts.push('document.getElementById("o-neg-reviews").textContent=negReviews>0?negReviews+" unanswered":"--";');
  parts.push('var repOpp="";');
  parts.push('if(replyRate!==null&&replyRate<30&&negReviews>0){repOpp=biz+" is currently responding to "+replyRate+"% of reviews, with "+negReviews+" negative reviews sitting unanswered. Every unanswered review is a missed chance to show potential customers how the business handles feedback, one of the strongest trust signals in local search.";}');
  parts.push('else if(replyRate!==null&&replyRate<50){repOpp="With a "+replyRate+"% response rate, there is a real opportunity to build more trust with potential customers. Businesses that respond consistently rank higher in local search and convert more first-time visitors into bookings.";}');
  parts.push('else if(gmbTotal>0){repOpp="The review foundation is solid with "+Number(gmbTotal).toLocaleString()+" Google reviews. The opportunity is in building volume and making sure every review gets a timely, on-brand response.";}');
  parts.push('else{repOpp="Building a strong review presence is one of the highest-impact steps a local business can take. Even a small number of recent, responded-to reviews can meaningfully improve local search ranking.";}');
  ...
  parts.push('heatmap.forEach(function(spot){');
  parts.push('if(spot.topBusinesses){spot.topBusinesses.forEach(function(b){if(b.name&&b.name!==biz&&!seen[b.name]){seen[b.name]=true;comps.push({name:b.name,reviews:b.userRatingsTotal||0});}});}');
  parts.push('});');
  parts.push('comps.sort(function(a,b){return b.reviews-a.reviews;});');
  parts.push('comps.slice(0,5).forEach(function(c){var tr=document.createElement("tr");tr.innerHTML="<td>"+c.name+"</td><td>"+Number(c.reviews).toLocaleString()+"</td>";tbody.appendChild(tr);});');
  ...
  parts.push('var hs=d.healthScore||{};');
  parts.push('var gbpScore=hs.scanPercentage!=null?Math.round(hs.scanPercentage):null;');
  ...
  parts.push('{label:"Photos",val:!!(d.gmbData&&d.gmbData.photos)},');
  parts.push('{label:"Hours of operation",val:!!(d.gmbData&&(d.gmbData.openingHours||d.gmbData.hours))},');
  ...
  parts.push('else if(gbpScore>=80){gbpOpp="At "+gbpScoreDisplay+", the profile is in solid shape. Ongoing maintenance, keeping photos fresh, responding to Q&As, and posting updates, is what separates businesses that hold their ranking from those that gradually slip.";}');
