function getHTML() {
  ...
  parts.push('<div class="stat-chip alert"><div class="val" id="o-rep-score"></div><div class="lbl">Reputation score</div></div>');
  ...
  parts.push('if(negReviews>3&&repScore!==null&&repScore<80){repOpp=biz+" has "+negReviews+" negative reviews and a reputation score of "+repScore+"%. Responding consistently to reviews, especially the critical ones, is one of the fastest ways to improve trust and local search ranking.";}');
  ...
  parts.push('heatmap.forEach(function(spot){if(spot.hotSpots){spot.hotSpots.forEach(function(hs){if(hs.name&&hs.name!==biz&&!seen[hs.name]){seen[hs.name]=true;comps.push({name:hs.name,reviews:hs.userRatingsTotal||0});}});}');
  parts.push('if(spot.topBusinesses&&spot.topBusinesses.length>0){spot.topBusinesses.forEach(function(b){if(b.name&&b.name!==biz&&!seen[b.name]){seen[b.name]=true;comps.push({name:b.name,reviews:b.userRatingsTotal||0});}});}');
  parts.push('});');
  ...
  parts.push('if(!hasWebsite&&gbpScore!==null){gbpOpp="The profile scores "+gbpScoreDisplay+" with the main gap being a linked website. Google uses the website connection as a trust and ranking signal. Adding one positions the business to rank higher for local searches.";}');
